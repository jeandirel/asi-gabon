  ...
  return (
    <div className="bg-surface text-on-surface antialiased min-h-screen pb-32">
      ...
      <main className="pt-24 px-6 max-w-7xl mx-auto">
        <section className="mb-16">
          ...
          <form action="/assistant" className="flex flex-col md:flex-row gap-4 mb-12">
            ...
            <div className="flex-grow flex items-center bg-surface-container-high/50 rounded-xl px-6 py-4 shadow-inner border border-outline-variant/20 focus-within:ring-2 focus-within:ring-primary/20 focus-within:bg-surface-container-lowest focus-within:border-primary/30 transition-all">
            ...
            <button className="bg-primary text-on-primary px-8 py-4 rounded-xl font-bold font-headline shadow-subtle hover:shadow-elevated transition-all active:scale-95" type="submit">
              Trouver
            </button>
          </form>
        </section>
        <section className="mb-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 md:gap-8">
            {SERVICE_CATEGORIES.map(([title, text, icon, colors, query]) => (
              <Link
                key={title}
                href={`/assistant?launch=1&q=${encodeURIComponent(`J'ai besoin d'aide pour ${title}. ${text}`)}`}
                className="group bg-surface-container-lowest p-8 rounded-2xl flex flex-col h-full border border-outline-variant/10 hover:border-primary/20 shadow-sm hover:shadow-elevated transition-all duration-300"
              >
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 shadow-sm group-hover:shadow-md transition-shadow ${colors}`}>
                ...
                <span className="w-full flex items-center justify-between py-3 px-4 rounded-xl bg-surface-container-low text-primary font-bold group-hover:bg-primary group-hover:text-on-primary transition-all">
                  Consulter
      ...
      <Link
        ...
        className="fixed bottom-24 right-6 md:bottom-8 md:right-8 w-16 h-16 rounded-full bg-primary text-on-primary shadow-elevated flex items-center justify-center hover:scale-105 active:scale-95 transition-all duration-300 z-40 group hover:shadow-[0_12px_36px_-4px_rgba(22,92,169,0.4)]"
      >
        <span className="material-symbols-outlined text-3xl">chat_bubble</span>
        <span className="absolute right-full mr-4 bg-on-surface text-surface text-xs font-bold py-2 px-4 rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity shadow-subtle">
          Une question ?
        </span>
      </Link>
    </div>
